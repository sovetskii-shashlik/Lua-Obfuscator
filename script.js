// --- Theme toggle logic ---
                 ...
                 if(m&&(m[1]!==undefined)&&(m[2]!==undefined)){
                    ...
                    for(let i=0;i<Math.max(a1.length,a2.length);i++){
                       ...
                       if(i<a2.length){
                           ...
                           else throw new Error(`Деobfuscation interleave: Неверный код в массиве b (${a2[i]}).`);
                       }
                    }
                    deobfuscated=true;
                }
    ...
    // --- Copy to Clipboard (Code Obfuscator) ---
     ...
     function encodeUrl() {
         ...
         // Iterate through the part of the URL *after* the scheme
         for (; i < url.length; i++) {
             const char = url[i];

             if (safeChars.has(char)) {
                 // If the character is in our custom safe list, append it directly
                 encodedUrlPart += char;
             } else {
                 // Check if it's a basic ASCII character (<= 255) or multi-byte
                 // charCodeAt(0) is sufficient here because we are explicitly handling
                 // characters <= 255 one way and > 255 the other.
                 const charCode = char.charCodeAt(0);

                 if (charCode <= 255) {
                     // If it's a single-byte character (like ASCII letters/numbers/symbols not in safeChars),
                     // encode it as %XX using its single code unit value, replicating the original behavior.
                     encodedUrlPart += "%" + charCode.toString(16).toUpperCase().padStart(2, '0');
                 } else {
                     // If it's a multi-byte character (like Cyrillic, emojis, etc.),
                     // use encodeURIComponent to get the correct UTF-8 multi-byte percent-encoding.
                     encodedUrlPart += encodeURIComponent(char);
                 }
             }
         }

         const finalUrl = scheme + encodedUrlPart; // Recombine scheme with the encoded part
